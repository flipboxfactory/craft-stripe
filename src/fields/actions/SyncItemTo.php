...
class SyncItemTo extends AbstractIntegrationItemAction
{
    ...
    {
        return Stripe::t('Sync To Stripe');
    }
    ...
    {
        return Stripe::t("Performing a sync will transmit any unsaved data.  Please confirm to continue.");
    }
    ...
    public function performAction(Integrations $field, ElementInterface $element, IntegrationAssociation $record): bool
    {
        ...
        if (!$field->syncToStripe($element)) {
            $this->setMessage("Failed to sync to Stripe " . $field->getObjectLabel());
            return false;
        }

        $this->setMessage("Sync to Stripe executed successfully");
        return true;
    }
}
